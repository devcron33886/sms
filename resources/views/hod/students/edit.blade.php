...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("hod.students.update", [$user->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('department') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($departments as $id => $entry)
                            <option
                                value="{{ $id }}" {{ (old('department_id') ? old('department_id') : $user->department->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
